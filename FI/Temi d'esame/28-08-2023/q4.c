/*
 * File: q4.c
 * Descrizione: Tema d'esame del 28 agosto 2023 - Questione 4
 *              Costruzione di lista concatenata basata su somme di coppie di elementi
 * Autore: Studente FI
 * Data: 28 agosto 2023
 * Contesto: Fondamenti di Informatica - Strutture dati dinamiche
 * 
 * Funzionalità:
 * - Crea una lista concatenata analizzando coppie di elementi adiacenti
 * - Memorizza indici in base al confronto con una somma target
 * - Gestisce allocazione dinamica di memoria per i nodi
 * 
 * Algoritmo:
 * - Se somma coppia = target: memorizza entrambi gli indici
 * - Se somma coppia > target: memorizza solo primo indice  
 * - Se somma coppia < target: memorizza solo primo indice
...
/**
 * Funzione per creare un nuovo nodo inizializzato
...
/**
 * Funzione principale per convertire array in lista basata su analisi di coppie
 * 
 * Algoritmo:
...
Node* ArrayPartToList(int my[], int n, int sum) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (i < n-1) {  // === GESTIONE COPPIE ===
            ...
            if (list == NULL) {
                list = current;        // Caso particolare: array di 1 elemento
            } else {
...
/**
 * Funzione principale - test dell'algoritmo di conversione array-lista
 */
int main(){
    // === DATI DI TEST ===
    ...
    // === OUTPUT ===
    // Stampa lastindex del secondo nodo (posizione 1 nella lista)
