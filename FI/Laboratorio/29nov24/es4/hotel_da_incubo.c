// hotel_da_incubo.c - Analisi voti hotel
...
float voto_hotel(char myhfilename[]){
    ...
    for (int i=0; i<people; i++) sum = sum + myvoti[i].serv;
    avgserv = (float) sum / people;
    sum = 0;

    for (int i=0; i<people; i++) sum = sum + myvoti[i].pul;
    avgpul = (float) sum / people;
    sum = 0;

    for (int i=0; i<people; i++) sum = sum + myvoti[i].pos;
...
char* incubo(char index[]){
    ...
    // Bubble sort: hotel con punteggio peggiore all'inizio
